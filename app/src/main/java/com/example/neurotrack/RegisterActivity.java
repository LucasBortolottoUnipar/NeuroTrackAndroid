...
public class RegisterActivity extends AppCompatActivity {
    ...
    private void performRegister() {
        ...
        if (age == null) {
            editTextData.setError("Data de nascimento inválida");
            editTextData.requestFocus();
            return;
        }
        if (age < 18) {
            editTextData.setError("Você deve ter pelo menos 18 anos para se cadastrar.");
            editTextData.requestFocus();
            return;
        }
    ...
    private boolean validateInputs(String nome, String email, String senha, String data) {
        ...
        if (nome.length() > 60) {
            editTextNome.setError("Nome deve ter no máximo 60 caracteres");
            editTextNome.requestFocus();
            return false;
        }
    ...
    private Integer calculateAge(String date) {
        try {
            ...
            birthCal.setLenient(false);
            birthCal.set(year, month, day);
            birthCal.getTime(); // force validation
